...
class Generator {
    ...
    async fetchApi(projectConfig = this.config) {
        ...
        const { _yapi_token, _yapi_uid } = login_1.default.getToken();
        const headers = {
    ...
    async generate() {
        if (!isGitWorkspaceEmpty) {
            consola_1.default.error('检测到您git工作区存在未提交文件，请先处理');
            ...
            utils_1.mkdirs(this.config.outputFilePath, () => {
                ...
                outputs.forEach((api, i) => {
                    ...
                    if (i === outputs.length - 1) {
                        ...
                        consola_1.default.warn(`project: ${this.config.projectId}, 共计更新了${addeds.length + deleteds.length + modifieds.length}个接口文件，请到git工作区比对文件更新`);
                        consola_1.default.log('===================================================');
