...
const openChangelog = (outputFilePath) => {
    ...
    const updateJson = fs_extra_1.default.readFileSync((0, utils_1.resolveApp)(`${outputFilePath}/update.json`)).toString();
    const port = Math.ceil(Math.random() * 10000);
    app.listen(port, () => {
        const uri = `http://localhost:${port}`;
        console.log(`变更日志：${uri}`);
        (0, open_1.default)(uri);
        app.get('/', (req, res) => {
            res.send((0, template_1.viewHtmlTemplate)(updateJson));
        });
    });
};
TSNode.register({
...
const generatoraFiles = async (config) => {
    ...
    if (!login_1.default.isLogin()) {
        yapiUserFile_1.default.setServer(config.serverUrl);
        await login_1.default.run(config.serverUrl);
    }
    const spinner = (0, ora_1.default)(`🛫 正在获取project: ${config.projectId} 的yapi数据样本`).start();
    try {
