...
function jsonToJsonSchema(json) {
    return processJsonSchema(json_schema_generator_1.default(json));
}
...
async function jsonSchemaToType(jsonSchema, typeName) {
    ...
    const code = await json_schema_to_typescript_1.compile(jsonSchema, typeName, JSTTOptions);
    return code.trim();
}
exports.jsonSchemaToType = jsonSchemaToType;
const appDirectory = fs_1.default.realpathSync(process.cwd());
exports.resolveApp = (relativePath) => path_1.default.resolve(appDirectory, relativePath);
exports.mkdirs = (dirpath, callback) => {
    ...
    if (exists) {
        ...
        //尝试创建父目录，然后再创建当前目录
        exports.mkdirs(path_1.default.dirname(dirpath), () => {
...
exports.writeFileSync = (dirpath, data) => {
