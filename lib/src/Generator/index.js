...
const readGitStatus = () => {
    ...
    // 获取git 工作区状态
    const stdout = child_process_1.execSync('git status --porcelain', {
        stdio: ['pipe', 'pipe', 'ignore'],
    }).toString().trim();
    const isGitWorkspaceEmpty = stdout === '';
    // 获取当前分支名称
    const branchName = child_process_1.execSync('git symbolic-ref --short -q HEAD').toString().trim();
...
class Generator {
    ...
    async fetchApi(projectConfig = this.config) {
        if (!isGitWorkspaceEmpty && !this.config.notCheckGit) {
            consola_1.default.error(`请先处理git工作区未提交文件，再运行 yapiMagic`);
            process.exit(0);
        }
        // 登录 获取用户信息
        ...
        const { _yapi_token, _yapi_uid } = await login_1.default(serverUrl);
        const headers = {
    ...
    compareApiFile(files, name, data) {
        ...
        if (matched.length > 0) {
            ...
            const data1 = prettier_1.default.format(data, {
            ...
            if (oldData !== data1) {
                ...
                utils_1.writeFileSync(utils_1.resolveApp(`${this.config.outputFilePath}/${name}.${this.config.target}`), data1);
            }
            else {
            ...
            utils_1.writeFileSync(utils_1.resolveApp(`${this.config.outputFilePath}/${name}.${this.config.target}`), prettier_1.default.format(data, {
                parser: this.config.target === 'ts' ? 'typescript' : 'babel',
            }));
    ...
    getfileDiff(oldStr, str) {
        return git_diff_1.default(oldStr, str, {
            color: false,
            save: true,
            wordDiff: false,
    ...
    getDeletedFiles(files, outputs) {
        ...
        files.forEach(file => {
            if (outputs.indexOf(file) === -1 && this.ignoreFiles.indexOf(file) === -1) {
                ...
                fs_extra_1.default.unlinkSync(utils_1.resolveApp(`${this.config.outputFilePath}/${file}`));
            }
        });
    }
    // 深度比较 不包含 time字段
    ...
    writeLog() {
        ...
        const fileName = utils_1.resolveApp(`${this.config.outputFilePath}/update.json`);
    ...
    write(outputsBase, callback) {
        ...
        try {
            ...
            utils_1.mkdirs(this.config.outputFilePath, () => {
                const files = fs_extra_1.default.readdirSync(utils_1.resolveApp(this.config.outputFilePath));
                // files里存在 outputs不存在 则为即将删除的文件
                // 1.1.0 采用不删除接口文件策略 由于删除是个低频且高风险操作
                // this.getDeletedFiles(files, outputs.map(output => `${output.name}.${this.config.target}`))
                if (outputs.length === 0) {
                    consola_1.default.info(`😄 customizeFilter 没有匹配接口，运行结束`);
                    return;
                }
                outputs.forEach((api, i) => {
                    ...
                    if (i === outputs.length - 1) {
                        ...
                        if (modifieds.length > 0) {
                            consola_1.default.log('---------------------------------------------------');
                            consola_1.default.warn(`❗ 更新接口：${modifieds.length} 个, 如下:`);
                        ...
                        consola_1.default.log('===================================================');
                        consola_1.default.warn(`共计更新了${addeds.length + deleteds.length + modifieds.length}个接口文件，请到git工作区比对文件更新`);
                        ...
                        if (this.config.generateIndexFile) {
                            ...
                            utils_1.mkdirs(this.config.outputFilePath, () => {
                                utils_1.writeFileSync(utils_1.resolveApp(`${this.config.outputFilePath}/index.${this.config.target}`), indexData);
                            });
                        }
